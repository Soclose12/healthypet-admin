...
function SettingsPage({ swal }) {
   ...
   return (
      <Layout>
         ...
         {!isLoading && (
            <>
               <label>Рекомендований продукт</label>
               <select value={featuredProductId} onChange={ev => setFeaturedProductId(ev.target.value)}>
                  {products.length > 0 && products.map(product => (
                     <option value={product._id}>{product.title}</option>
                  ))}
               </select>
               <label>Вартість доставки (в доларах США)</label>
               <input type="number"
                  value={shippingFee}
                  onChange={ev => setShippingFee(ev.target.value)}
               />
               <div>
                  <button onClick={saveSettings} className="btn-primary">Зберегти налаштування</button>
               </div>
            </>
         )}
      </Layout>
